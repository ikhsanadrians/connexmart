...
<div class="headers flex justify-between">
    <h1 class="text-2xl font-bold">Kas/Shift Kasir</h1>
    <a href="{{ route("mart.cashier.shift.history") }}" class="text-blue-700">Lihat History Kas/Shift</a>
</div>
@if(!$cashierShift)
<form method="POST" action="{{ route("mart.cashier.shift.post")}}" class="add-shift mt-4">
    @csrf
    ...
    <div class="starting-cash">
        <label for="">Jumlah Awal Kas</label>
        <input 
        class="w-full rounded-md focus:outline-none focus:ring-2 ring-[#303fe2] bg-gray-100 my-2 p-2"
        type="text" name="startCash" id="goods" placeholder="Masukan Jumlah Awal Kas">
    </div>
    <button type="submit" class="bg-gradient-to-r from-blue-600 to-blue-700 px-8 float-right mt-2 py-3 text-white font-semibold rounded-md">
        Mulai Shift
    </button>
</form>
@else 
<div class="shift-running flex flex-col bg-white p-8 mt-6 rounded-lg">
    ...
    <form action="{{ route("mart.cashier.shift.end") }}" method="POST">
        ...
        <button type="submit" class="bg-gradient-to-r  from-red-600 to-red-700  px-8 w-full mt-6 py-3 text-white font-semibold rounded-md">
            Hentikan Shift
        </button>  
    </form>
</div>
