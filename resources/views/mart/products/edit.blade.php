...
@section('content')
    ...
    <div class="headers flex justify-between mt-4">
        <h1 class="text-2xl font-bold">Edit Produk / Barang {{ $product->name }}</h1>
    </div>
    <form enctype="multipart/form-data" action="{{ route('mart.updategoods', $product->id) }}" method="POST"
        class="products-container w-full mt-5 bg-white mb-12 px-6 pt-6 pb-8 rounded-lg flex flex-col gap-3">
        @csrf
        @method('put')
        <div class="input-name flex flex-col mt-2 space-y-2">
            <label class="font-medium" for="">Nama Produk</label>
            <input value="{{ $product->name }}" name="name" type="text" placeholder="Masukan Nama Produk"
        ...
        <div class="input-barcode flex flex-col mt-2 space-y-2">
            ...
            <input value="{{ $product->barcode_number }}" name="barcode_number" type="text"
                placeholder="Masukan Nomor Barcode" class="outline-none px-4 py-3 bg-gray-100 rounded-md">
        </div>
        <div class="input-price flex flex-col mt-2 space-y-2">
            ...
            <input value="{{ $product->price }}" name="price" type="text"
                placeholder="Masukan Harga Produk, Contoh: 6500" class="outline-none px-4 py-3 bg-gray-100 rounded-md">
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                ...
                <select id="category-input"
                    ...
                    @foreach ($productcategories as $productcategory)
                        <option {{ $product->category_id == $productcategory->id ? 'selected' : '' }}
                            value="{{ $productcategory->id }}">{{ ucfirst($productcategory->name) }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="input-stock flex flex-col mt-2 space-y-2">
                ...
                <input value="{{ $product->stock }}" name="stock" type="text" placeholder="Masukan Stok Produk"
                    class="outline-none px-4 py-3 bg-gray-100 rounded-md">
            </div>
        </div>
        <div class="input-description flex flex-col mt-2 space-y-2">
            ...
            <textarea name="description" id="" class="w-full rounded-md focus:outline-none bg-gray-100 my-2 p-3"
                placeholder="Masukan Deskripsi">{{ $product->desc }}</textarea>
        </div>
        <div class="input-images">
            ...
            <div class="input-images mt-2 flex items-center w-full h-12 bg-gray-100 relative">
                <div class="img-previews h-12 w-12 z-20">
