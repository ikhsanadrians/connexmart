...
<div class="headers flex items-center gap-3">
    <a href="{{ route("mart.cashier.shift") }}" class="back bg-blue-600 w-fit px-3 rounded-md py-2 text-white font-semibold">Kembali</a>
    ...
    <div class="header flex justify-between items-center gap-3 my-2">
        <div class="header-left flex justify-between items-center gap-3">
            ...
            <div class="times flex items-center gap-1 text-sm">
                ...
                <p class="time-text">
                    {{ $cashierShift->updated_at->format('d M Y; h:m') }}
                </p>
            </div>
            <div class="status">
                @if($cashierShift->status == "current")
                  <p class="bg-gradient-to-r from-green-500 to-emerald-600 px-3 py-1 text-white text-sm font-semibold rounded-md">
                      Sedang Berjalan
                  </p>
                @else 
                  <p class="bg-gradient-to-r from-blue-500 to-blue-600 px-3 py-1 text-white text-sm font-semibold rounded-md">
                      Berakhir 
                  </p>
                @endif
            </div>
                ...
                <tr class="!border-none">
                    ...
                    <td class="!border-none text-start">
                        <div class="quantity">
                            ...
                            <p class="text-center">{{ $cashierShift->sold_items }}</p>
                        </div>
                    </td>
                    <td class="!border-none text-start">
